...
date: 2026-09-14
category: 学习记录/数学
description: 一对一学拉格朗日插值的复盘。卡住的四个疑问、开关和地雷的直觉、思考题的幸存者法则，以及误差公式为什么非要长成那个样子。
top: false
updates:
  - 2026-09-14 首版发布
---
...
还有一个更狠的视角，是一句话把我打通的：**别把开关看成 n 个分式相乘，把它看成一个分数——分子是手指的脚印，分母是自己钉子的脚印，读数就是两个脚印的重合度。** 分母是死标尺，分子是活测量，比值天然在别人家归零、在自己家满格、半路连续过渡。曲线不是画出来的，是这个比值结构自己长出来的。
...
页尾思考题要证：ω′ₙ₊₁(xᵢ) 等于那串跳过 xᵢ 的连乘。我第一次的回答是「求导，然后代入 xᵢ」——动作对了，但老师追问「为什么代进去只剩一串连乘」，我答不上来。
...
这和开关里「踩别人爆雷」是同一个机关，只是从乘法搬家到了求导。

## ω 和误差公式：不是换皮，是换舞台

ωₙ₊₁(x) 是全节点大连乘，一个因子不跳。我一开始以为它只是把 4.11 换个写法，老师说对，但有三个真金白银的用处：
...
误差公式每个符号的角色，我按闭环标准拆一遍：ξ 读「克赛」，是区间内某个存在但位置未知的中值定理产物，所以那块算不出精确值只能框上限；分母 (n+1)! 是点越多压得越狠的压制项；ω 衡量「你离钉子阵整体有多远」，踩中任何钉子它归零，误差也归零——替身在钉子上本来就是完美的。
...
展开验货：L₂(x) = −1.5x² + 3.5x + 1，代 0、1、2 回去正好是 1、3、2，三颗钉子分毫不差。
:::

四个自测题全过之后，这课算闭环了。下一站两个候选：插值多项式的唯一性证明，或者切比雪夫节点怎么让 |ω| 最小。等下次课。
